<template>
  <el-dialog :title="_label('createProduct')" :visible.sync="dialogVisible" :center="true" width="1400px"
             :modal="true"
      ...
      <el-row class="product" style="margin-bottom: 20px;">
        ...
        <el-col :span="24">
          ...
          <el-form-item :label="showLabel('xingbie')">
            <sp-radio-group v-model="product.form.gender" source="gender" :span="8" :lang="lang" class="supermini"
                            style="width:270px">
            </sp-radio-group>
          </el-form-item>
          <!-- 性别 end -->
...
<script>
    ...
    import size from '@/view/product/components/size.vue'
    import sizetop from '@/view/product/components/sizetop.vue'
    import productMemo from '@/view/product/components/productMemo.vue'
    import {_label} from "@/component/globals"
    import {extend} from "@/component/object"
    ...
    export default {
        ...
        methods: {
            ...
            setForm(info) {
                let self = this
                // 把当前的 info 传递给 product
                extend(self.product, info)
                return self
            },
            filterColors(row) {
            ...
            handleAppendMaterial() {
                // 如果没有选择品类，那么就报错
                if (!this.product.form.brandgroupid || this.product.form.brandgroupid.length === 0) {
