<template>
  <div>
    <el-form class="order-form" :model="form" label-width="85px" :inline="true" style="width:100%;" size="mini">
      <!-- 预售 + 销售 start -->
      <el-row :gutter="0">
        <asa-button @click="sale(1)" :enable="_isAllowed('sales-add') && tabledata.length>0">{{ _label("yushou") }}
        </asa-button>
        <asa-button @click="sale(2)" :enable="_isAllowed('sales-add') && tabledata.length>0">{{ _label("xiaoshou") }}
        </asa-button>
      </el-row>
      <!-- 预售 + 销售 end -->
      ...
      <el-row :gutter="0">
        <el-col :span="4" style="width:300px">
          <el-form-item :label="_label('xiaoshouduankou')">
            <simple-select ref="saleportid" v-model="form.saleportid" source="usersaleport"
                           :clearable="false"></simple-select>
          </el-form-item>
          <el-form-item :label="_label('jiage')">
            <simple-select ref="priceid" v-model="form.priceid" source="userprice" @change="onPriceChange"
                           :disabled="form.status!=0" :clearable="false"></simple-select>
          </el-form-item>
          <el-form-item :label="_label('xiaoshoucangku')">
            <simple-select v-model="form.warehouseid" source="userwarehouse" :disabled="form.status!=0"
                           :clearable="false"></simple-select>
          </el-form-item>
      ...
      <el-table :data="tablerows" stripe border style="width:100%;" v-if="form.warehouseid>0">
        <el-table-column :label="_label('guojima')" align="left" width="250">
          <template v-slot="{row}">
            ...
            <el-autocomplete ref="wordcode" v-model="search.wordcode" :placeholder="_label('guojima')"
                             :fetch-suggestions="searchProduct" @select="onSelect" value-key="wordcode"
                             v-if="row.type=='foot'" size="mini" style="width:100%">
            </el-autocomplete>
          </template>
        </el-table-column>
        <el-table-column :label="_label('chima')" width="100" align="center">
          <template v-slot="{row}">
            ...
            <el-select ref="select" v-model="search.sizecontentid" :placeholder="_label('chima')" style="width:150"
                       v-if="row.type=='foot'" size="mini">
              <el-option v-for="item in options" :key="item.id+item.name" :value="item.id"
        ...
        <el-table-column :label="_label('shangpintiaoma')" align="center" width="150">
          <template v-slot="{row}">
            <sp-product-code :productid="row.product.id" :sizecontentid="row.sizecontentid"
                             v-if="!row.type"></sp-product-code>
            <el-input ref="goods_code" v-model="search.goods_code" :placeholder="_label('shangpintiaoma')"
                      v-if="row.type=='foot'" size="mini"></el-input>
          </template>
        </el-table-column>

        <el-table-column :label="_label('shuliang')" width="70" align="left">
          <template v-slot="{row}">
            <el-input v-model="row.number" size="mini" :disabled="form.status>=2" v-if="!row.type" class="number"
                      @focus="onFocus(row.index)" :ref="row.index"></el-input>

            <el-input v-model="search.number" :placeholder="_label('shuliang')" v-if="row.type=='foot'"
                      size="mini"></el-input>
          </template>
        </el-table-column>
        ...
        <el-table-column :label="_label('chengjiaojia')" width="90" align="center">
          <template v-slot="{row}">
            ...
            <el-button type="primary" style="margin:0px" native-type="submit" @click="onClick" v-if="row.type=='foot'"
                       size="mini">{{ _label('zengjia') }}
            </el-button>
          </template>
        </el-table-column>
        ...
        <el-table-column :label="_label('xuanze')" width="70" align="center">
          <template v-slot="{row}">
            <sp-sales-select v-model="row.warehouselist" :stocks="warehouseStocks[row.index]"
                             :warehouseid="form.warehouseid" :number="row.number" v-if="!row.type"
                             :is-dispatch="dispatches[row.index]==row.number"></sp-sales-select>
          </template>
        </el-table-column>
        ...
        <el-table-column :label="_label('caozuo')" width="80" align="center" v-if="form.status==0 || form.status==1">
          <template v-slot="{row,$index}">
            <as-button size="mini" type="danger" @click="deleteRow($index, row)" v-if="!row.type">
              {{ _label('shanchu') }}
            </as-button>
          </template>
        </el-table-column>
